...
namespace UnityEngine
{
	...
	public class RuleTile : TileBase
	{
			...
			for (int y = -1; y <= 1; y++)
			{
				for (int x = -1; x <= 1; x++)
				{
					if (x != 0 || y != 0)
					{
						...
						if(NeighbouringTileFitsRuleExp(rule.m_Neighbors[index],tile)) return false;
					}
				}
		...
		private bool NeighbouringTileFitsRuleExp(TilingRule.Neighbor neighbourExp, TileBase neighbourTile) {
			TileBase thisTile = this;
			if(m_onlySameTile) {
				if (neighbourExp == TilingRule.Neighbor.This && neighbourTile != this || neighbourExp == TilingRule.Neighbor.NotThis && neighbourTile == this)
					return true;
			} else {
				if (neighbourExp == TilingRule.Neighbor.This && neighbourTile == null || neighbourExp == TilingRule.Neighbor.NotThis && neighbourTile != null)
					return true;
			}
